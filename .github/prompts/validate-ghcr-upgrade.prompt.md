---
description: "Validate whether this fork can safely update its GHCR images after upstream main/staging changes. Use when reviewing the pushed GHCR codebase, auditing major upgrades such as v0.28, comparing the fork with upstream main or staging, deciding whether to reuse upstream GHCR or rebuild fork GHCR, asking whether fork/main or local containers can pin to upstream GHCR from now on, or checking whether docker-compose, Watchtower, installers, release notes, and release assets will actually pick up the update."
---

# Validate GHCR Upgrade

Use this when you need to answer: "Can we update the fork GHCR image from upstream changes, can fork/main or local image pins move to upstream GHCR, and will the downstream install/update paths actually pick it up?"
...
- Do not answer "yes, pin to upstream GHCR" unless the audit proves the runtime image, worker image, compose or `.env` pins, and installer or release-channel surfaces stay coherent for the targeted release line.
...
- [Local ops rules](../instructions/local-ops.instructions.md)
...
- Whether `IRONCLAW_APP_IMAGE`, `SANDBOX_IMAGE`, or `docker-compose.yml` defaults would need to move from fork GHCR to upstream GHCR, and whether that changes auth, rollback, or automation expectations.
...
- `Safe to pin fork/main or local containers to upstream GHCR` only if the required upstream runtime and worker tags or digests exist, local pins and compose defaults can move without losing rollback clarity, and fork-only publication or release-channel behavior is either preserved elsewhere or intentionally retired.
