...
services:
  ...
  # Optional: pins the sandbox worker image with an idle container so
  # Docker Desktop cache cleanup is less likely to remove it between jobs.
  # This does not replace IronClaw's normal on-demand worker containers.
  worker-cache:
    profiles: ["worker-cache"]
    image: ghcr.io/jzkk720/ironclaw-worker:latest
    restart: unless-stopped
    entrypoint: ["/bin/sh", "-lc", "exec tail -f /dev/null"]
    read_only: true
...
### Optional: Pin the Worker Image

If you use Docker sandbox jobs on Docker Desktop and frequently clear images or caches,
you can keep the worker image referenced by an idle container:

```bash
docker compose --profile worker-cache up -d worker-cache
```

This only pins `ghcr.io/jzkk720/ironclaw-worker:latest` in the local Docker cache.
IronClaw still launches a fresh worker container per sandbox job, and the app's
auto-pull logic remains the real recovery path if the image is ever removed.

To stop the cache container later:

```bash
docker compose --profile worker-cache rm -sf worker-cache
```
