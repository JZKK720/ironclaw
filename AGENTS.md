...
- On Windows, prefer Docker validation over host Rust execution. Use `docker compose up -d --build ironclaw worker-cache` for local rebuilds, and use `docker compose pull` plus `docker compose up -d --no-build postgres ironclaw worker-cache` when the task is to validate pulled GHCR images instead of rebuilding.
