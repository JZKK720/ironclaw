...
| Prompt | Purpose |
|--------|---------|
| `/plan-upstream-upgrade` | Planning-first guide to compare a targeted upstream release with origin/main, classify fork-only commits as keep/reapply/replace/defer, decide the GHCR publication path, and prepare local Docker rollout notes before executing anything |
...
| `/validate-ghcr-upgrade` | Audit whether the fork should publish its own GHCR images or can safely pin fork/main and local containers to upstream GHCR, and whether downstream update/install channels will actually pick that up |
