...
services:
  ironclaw:
    ...
    environment:
      ...
      # If you enable a proxy, keep the hosted ClawHub registry hosts out of it
      # or skills search/install can fail with "Registry unreachable".
      # - HTTP_PROXY=http://host.docker.internal:7897
      # - HTTPS_PROXY=http://host.docker.internal:7897
      # - NO_PROXY=host.docker.internal,localhost,127.0.0.1,wry-manatee-359.convex.site,clawhub.ai
...
> If `docker pull` or `docker compose pull` fails against GHCR, make sure the Docker daemon can resolve and reach both `ghcr.io` and `pkg-containers.githubusercontent.com`. GHCR serves manifests from `ghcr.io`, but layer blobs are downloaded from `pkg-containers.githubusercontent.com`.

> If the app runs behind an outbound HTTP(S) proxy, add `wry-manatee-359.convex.site` and `clawhub.ai` to `NO_PROXY`. Otherwise ClawHub skills search/install can fail from inside the container even when the image itself is up to date.
...
<AccordionGroup>
  ...
  <Accordion title="GHCR pull fails" icon="globe">
    Check Docker daemon DNS and outbound HTTPS for both hosts:
    ```bash
    nslookup ghcr.io
    nslookup pkg-containers.githubusercontent.com
    docker pull ghcr.io/jzkk720/ironclaw:latest
    ```
  </Accordion>
</AccordionGroup>
