...
# ─── Skills ───────────────────────────────────────────────────────────────────
SKILLS_ENABLED=true
# Leave CLAWHUB_REGISTRY unset for the default hosted ClawHub backend.
# Only set this when pointing at a compatible private registry.
# CLAWHUB_REGISTRY=https://wry-manatee-359.convex.site

# ─── Optional Outbound Proxy ──────────────────────────────────────────────────
# If IronClaw runs behind an HTTP(S) proxy, bypass the hosted ClawHub registry
# hosts or skills search/install can fail with "Registry unreachable".
...
> If `docker compose pull` fails against GHCR, fix Docker daemon DNS first. The daemon must be able to resolve and reach both `ghcr.io` and `pkg-containers.githubusercontent.com` because GHCR serves manifests and blobs from different hosts.

> If you set `HTTP_PROXY` or `HTTPS_PROXY` for the IronClaw container, add `wry-manatee-359.convex.site` and `clawhub.ai` to `NO_PROXY`. Otherwise server-side ClawHub skills search and install requests can fail with `Registry unreachable` even when the app image is current.
