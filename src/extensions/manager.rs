...
mod tests {
    ...
    fn always_present_env_var() -> &'static str {
    ...
    /// Uses a platform-stable env var to avoid `set_var`, which is unsafe in
    ...
    async fn test_env_var_token_skips_scope_expansion() -> Result<(), String> {
        ...
        let caps = serde_json::json!({
            "auth": {
                ...
                "env_var": always_present_env_var()
            }
        });
        ...
        // No managed token in secrets store — only the env var is present.
    ...
    async fn test_managed_token_takes_priority_over_env_var() -> Result<(), String> {
        ...
        // Both tools point env_var at a stable platform env var so the env-var path
