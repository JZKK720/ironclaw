...
fn read_locked_file(file: &mut fs::File) -> Result<String, PairingStoreError> {
...
fn write_locked_file(file: &mut fs::File, content: &str) -> Result<(), PairingStoreError> {
    file.set_len(0)?;
    file.seek(SeekFrom::Start(0))?;
...
impl PairingStore {
    ...
    fn record_failed_approve(&self, channel: &str) -> Result<(), PairingStoreError> {
        ...
        let mut data: ApproveAttemptsFile = read_locked_file(&mut file)
            .ok()
        ...
        write_locked_file(&mut file, &json)?;
        fs4::FileExt::unlock(&file)?;
        Ok(())
    }
