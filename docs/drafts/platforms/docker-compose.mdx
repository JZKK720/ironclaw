...
services:
  ...
  # Keeps the sandbox worker image pinned with an idle container so Docker
  # Desktop cache cleanup is less likely to remove it between jobs.
...
The stack starts `worker-cache` by default so `ghcr.io/jzkk720/ironclaw-worker:latest`
stays referenced by an idle container instead of showing up as an unused image.
...
If you need to stop the cache container temporarily:

```bash
docker compose stop worker-cache
```
